...
func (dialer Dialer) dial(ctx context.Context, address string) (net.Conn, error) {
	...
	if udpConn, ok := conn.(*net.UDPConn); ok {
		// Bump the OS-level socket receive buffer so that oversized UDP
		// datagrams (sent by some proxies/hosts) are accepted by the kernel
		// rather than dropped with EMSGSIZE("message too long"). The kernel
		// clamps this to the configured maximum, so a large value is safe.
		_ = udpConn.SetReadBuffer(readBufSize)
	}
...
func (dialer Dialer) DialContext(ctx context.Context, address string) (*Conn, error) {
	...
	if err = cs.discoverMTU(ctx); err != nil {
		return nil, dialer.error("dial", fmt.Errorf("discover mtu[go-raknet-fork]: %w", err))
	} else if err = cs.openConnection(ctx); err != nil {
		return nil, dialer.error("dial", fmt.Errorf("open connection[go-raknet-fork]: %w", err))
	}
